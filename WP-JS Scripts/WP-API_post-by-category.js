...
function getData(category) {
  ...
  var target = category;
  var targetUrl = url + "&categories=" + target;

  if(countCat > 1) {
    var pagination = "&page=" + (countCat+1);
    var pag_dest = targetUrl + pagination + cacheStamp;
  } else {
    ...
    $.getJSON(pag_dest, function (result) {
      if (!result.length) {
        ...
        $.each(result, function(i) {
          if(result[i].categories[0] !== 221 || result[i].categories[0] !== target) {
            buildCard(result[i]);
          } 
        });
      }
...
function buildCard(data) {
  ...
  var dataPrint = "";
      ...
      dataPrint += '<a class="display_thumb_size" href="'+ data.link +'" data-ajax="false"><img class="m_img ap-left ap-image-alt" src="'+ data._embedded["wp:featuredmedia"][0].source_url +'" alt=""></a>';
      ...
      dataPrint += !data._embedded.author[0]["author-title"] ? ',</span>' : '</span>';
      dataPrint += !data._embedded.author[0]["author-title"] ? "" : '<br><span class="ap-large-2">'+ data._embedded.author[0]["author-title"] +'</span>';
